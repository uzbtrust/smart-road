...
hang burns the weekly GPU quota. Two processes pinned to CUDA_VISIBLE_DEVICES=0
and =1 use both cards, cannot deadlock on NCCL, and hand us two heterogeneous
models -- which is what we want anyway, since ensembling different architectures
is what separated first place from tenth in CRDDC'2022.
...
JOBS = [
    {"name": "yolo11s_640", "model": "yolo11s.pt", "imgsz": 640, "batch": 32, "gpu": 0},
    {"name": "yolo11m_1024", "model": "yolo11m.pt", "imgsz": 1024, "batch": 8, "gpu": 1},
]
...
def find_data_yaml(explicit: str | None) -> Path:
    if explicit:
        ...
        return p
    candidates = sorted(KAGGLE_INPUT.glob("*/data.yaml")) + sorted(KAGGLE_INPUT.glob("*/*/data.yaml"))
    if not candidates:
...
def build_command(job: dict, data_yaml: Path, hours: float, epochs: int, patience: int, seed: int) -> list[str]:
    return [
        ...
        (
            ...
            f"imgsz={job['imgsz']},"
            f"batch={job['batch']},"
            "device=0,"              # index within CUDA_VISIBLE_DEVICES
